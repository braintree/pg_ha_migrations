  ...
  ["bogus_table", :bogus_table, "public.bogus_table"].each do |table_name|
    describe "#safely_acquire_lock_for_table #{table_name} of type #{table_name.class.name}" do
      let(:alternate_connection_pool) do
        ActiveRecord::ConnectionAdapters::ConnectionPool.new(DatabaseHelper.pool_config)
      end
      ...
      it "acquires an exclusive lock on the table by default" do
        migration.safely_acquire_lock_for_table(table_name) do
          expect(DatabaseHelper.locks_for_table(table_name, connection: alternate_connection)).to contain_exactly(
            having_attributes(
              table: "bogus_table",
      ...
      it "acquires exclusive locks by default when multiple tables provided" do
        migration.safely_acquire_lock_for_table(table_name, "bogus_table_2") do
          ...
          expect(DatabaseHelper.locks_for_table("bogus_table_2", connection: alternate_connection)).to contain_exactly(
            having_attributes(
              table: "bogus_table_2",
              lock_type: "AccessExclusiveLock",
              granted: true,
      ...
      it "releases the lock even after an exception" do
        ...
        expect(DatabaseHelper.locks_for_table(table_name, connection: alternate_connection)).to be_empty
      end
      ...
      it "waits to acquire a lock if the table is already blocked" do
        ...
        migration.suppress_messages do
          migration.safely_acquire_lock_for_table(table_name) do
            expect(DatabaseHelper.locks_for_table(table_name, connection: alternate_connection)).not_to be_empty
          end
        end
      end
          ...
          migration.suppress_messages do
            migration.safely_acquire_lock_for_table(table_name, "bogus_table_2") do
              aggregate_failures do
                expect(DatabaseHelper.locks_for_table(table_name, connection: alternate_connection_2)).not_to be_empty
                expect(DatabaseHelper.locks_for_table("bogus_table_2", connection: alternate_connection_2)).not_to be_empty
              end
            end
          end
        ensure
        ...
        begin
          ...
          migration.suppress_messages do
            migration.safely_acquire_lock_for_table(table_name, mode: :access_share) do
              locks_for_table = DatabaseHelper.locks_for_table(table_name, connection: alternate_connection)
      ...
      it "does not wait to acquire a lock if a table with the same name but in different schema is blocked" do
        ...
        begin
          ...
          migration.suppress_messages do
            migration.safely_acquire_lock_for_table(table_name) do
              locks_for_table = DatabaseHelper.locks_for_table(table_name, connection: alternate_connection)
              locks_for_other_table = DatabaseHelper.locks_for_table("partman.bogus_table", connection: alternate_connection)
      ...
      it "waits to acquire a lock if the table is partitioned and child table is blocked" do
        ...
        DatabaseHelper.create_range_partitioned_table(table_name, migration_klass, with_partman: true)
        ...
        begin
          ...
          migration.suppress_messages do
            migration.safely_acquire_lock_for_table(table_name) do
              locks_for_parent = DatabaseHelper.locks_for_table(table_name, connection: alternate_connection)
              locks_for_child = DatabaseHelper.locks_for_table("bogus_table_default", connection: alternate_connection)
      ...
      it "waits to acquire a lock if the table is partitioned and child sub-partition is blocked" do
        ...
        ActiveRecord::Base.connection.drop_table(table_name)
        DatabaseHelper.create_range_partitioned_table(table_name, migration_klass)
        DatabaseHelper.create_range_partitioned_table("#{table_name}_sub", migration_klass, with_partman: true)
        ActiveRecord::Base.connection.execute(<<~SQL)
        ...
        begin
          ...
          migration.suppress_messages do
            migration.safely_acquire_lock_for_table(table_name) do
              locks_for_parent = DatabaseHelper.locks_for_table(table_name, connection: alternate_connection)
              locks_for_sub = DatabaseHelper.locks_for_table("bogus_table_sub_default", connection: alternate_connection)
      ...
      it "waits to acquire a lock if the table is non-natively partitioned and child table is blocked" do
        ...
        begin
          ...
          migration.suppress_messages do
            migration.safely_acquire_lock_for_table(table_name) do
              locks_for_parent = DatabaseHelper.locks_for_table(table_name, connection: alternate_connection)
              locks_for_child = DatabaseHelper.locks_for_table("bogus_table_child", connection: alternate_connection)
      ...
      it "allows re-entrancy when multiple tables provided" do
        ...
        expect(DatabaseHelper.locks_for_table(table_name, connection: alternate_connection)).to be_empty
        expect(DatabaseHelper.locks_for_table("bogus_table_2", connection: alternate_connection)).to be_empty
      end
      ...
      it "does not allow re-entrancy when lock escalation detected" do
        expect do
          migration.safely_acquire_lock_for_table(table_name, mode: :share) do
            ...
            expect(DatabaseHelper.locks_for_table(table_name, connection: alternate_connection)).to_not be_empty
      ...
      it "skips blocking query check for nested lock acquisition" do
        ...
        begin
          migration.safely_acquire_lock_for_table(table_name) do
            ...
            migration.safely_acquire_lock_for_table(table_name) do
              expect(DatabaseHelper.locks_for_table(table_name, connection: alternate_connection_2)).to contain_exactly(
                having_attributes(
                  table: "bogus_table",
                  lock_type: "AccessExclusiveLock",
                  granted: true,
